...
import com.fututel.service.DeviceStateReceiver;
import com.fututel.service.Downloader;
...
public class NightlyUpdater {
	...
	private int getLastOnlineVersion() {
		try {
			URL url = new URL(BASE_UPDATE_VERSION + channel + "/Fututel-latest-" + channel + ".version");
	...
	private File getCachedFile() {
		return new File(context.getCacheDir(), "Fututel-latest-trunk.apk");
	}
				...
				ab.setMessage("Revision " + version + " available. Upgrade now?")
					.setPositiveButton(R.string.ok, new OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							it.setData(Uri.parse(BASE_UPDATE_VERSION + channel + "/Fututel-r"+version+"-" + channel + ".apk"));
							it.setClass(context, Downloader.class);
							it.putExtra(Downloader.EXTRA_ICON, R.drawable.ic_launcher_nightly);
							it.putExtra(Downloader.EXTRA_TITLE, "Fututel nightly build");
